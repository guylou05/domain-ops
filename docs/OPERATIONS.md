...
GitHub runs static/build, browser workflow, query profile, and asset-budget gates. The public Production smoke workflow runs daily. The authenticated production canary runs after each `main` push and every six hours with an OIDC-authorized ephemeral `VIEWER` credential, manages a single failure issue, and can also be started manually. See `docs/PRODUCTION-CANARY.md` for rotation and incident handling.
